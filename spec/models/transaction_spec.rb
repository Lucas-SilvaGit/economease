...
RSpec.describe Transaction, type: :model do
  ...
  before do
    ...
    create(:transaction, account: account, category: category, transaction_type: "income", due_date: 3.days.ago)
  ...
  describe "scopes" do
    it "returns only income transactions" do
      expect(Transaction.income.count).to eq(2)
    ...
    it "returns transactions for a specific user ordered by due_date desc" do
      ...
      transactions.each_cons(3) do |a, b|
        expect(a.due_date).to be > b.due_date
      end
    end
  end
end
